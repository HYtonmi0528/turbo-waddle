...
$proc = Start-Process -FilePath $mysqld -ArgumentList @("--defaults-file=$defaults", '--skip-grant-tables', '--console') -PassThru -WindowStyle Hidden -RedirectStandardOutput $stdout -RedirectStandardError $stderr
try {
...
FLUSH PRIVILEGES;
"@
  $sql | & $mysql --protocol=tcp -h 127.0.0.1 -u root
