      ...
      {tab === 'entry' ? (
        <section className="card collab-entry-card">
          ...
          {selectingItemId && (() => {
            ...
            return <div className="collab-candidate-picker"><div className="card-header"><div><h3>为第{target?.lineNo}项勾选供应商</h3><p className="text-muted text-sm">数据来自“{selectedQuoteSet?.name}”，按型号和描述优先排序。</p></div><button className="btn btn-outline btn-sm" onClick={() => setSelectingItemId('')}>关闭</button></div><div className="table-container"><table className="data-table"><thead><tr><th>选择</th><th>供应商</th><th>型号</th><th>含税运人民币</th><th>自动FOB</th><th>备注</th></tr></thead><tbody>{ranked.map(({ candidate, score }) => { const rmb = candidateRmbValue(candidate, selectedQuoteSet); const fob = rmb / (numberValue(exchangeRate) || 7.25) / (invoiceType === 'special' ? 1.13 : 1); return <tr key={`${candidate._quoteEntryId}-${candidate._quoteItemIndex}`}><td><button className="btn btn-primary btn-sm" disabled={savingId === target?.id} onClick={() => applyCandidate(target, candidate)}>✓ 选用</button></td><td>{candidate.supplierName || '—'}</td><td>{candidate.model || candidate.reference || '—'}</td><td>¥{rmb.toFixed(2)}</td><td>${fob.toFixed(2)}</td><td>{candidate.notes || candidate.afterSales || '—'}{score > 0 && <span className="rfq-match-score matched"> {score}分</span>}</td></tr>; })}</tbody></table></div></div>;
          })()}
